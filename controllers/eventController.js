...
//     // Replace images/videos only if new ones uploaded
//     if (newImages.length > 0) event.images = [...newImages];
//     if (newVideos.length > 0) event.videos = [...newVideos];
...
export const updateEvent = async (req, res) => {
  try {
    const { eventTitle, eventDate, description, time, existingImages, existingVideos } = req.body;
    ...
    // Handle images - use existing ones or merge with new ones
    if (existingImages) {
      // Parse existingImages if it's a string (from form data)
      const parsedExistingImages = typeof existingImages === 'string' 
        ? JSON.parse(existingImages) 
        : existingImages;
      event.images = [...parsedExistingImages, ...newImages];
    } else if (newImages.length > 0) {
      event.images = [...event.images, ...newImages];
    }

    // Handle videos - use existing ones or merge with new ones
    if (existingVideos) {
      const parsedExistingVideos = typeof existingVideos === 'string' 
        ? JSON.parse(existingVideos) 
        : existingVideos;
      event.videos = [...parsedExistingVideos, ...newVideos];
    } else if (newVideos.length > 0) {
      event.videos = [...event.videos, ...newVideos];
    }
